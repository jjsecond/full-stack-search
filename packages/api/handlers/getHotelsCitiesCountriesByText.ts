...
const getHotelsCitiesCountriesByText = async (req: Request, res: Response) =>{        
        try {
            ...
            const [countries, cities, hotels] = await Promise.all([
                db.collection(COLLECTION_NAMES.COUNTRIES).find({
                    country: {$regex: partialSearchText }
                   }).limit(10).toArray(),
                db.collection(COLLECTION_NAMES.CITIES).find({
                 name: {$regex: partialSearchText }
                }).limit(10).toArray(),
                db.collection(COLLECTION_NAMES.HOTELS).find(
                    {$or: [
            ...
            console.log('Error searching for hotels,cities and countries by text ', error);
            res.status(500).json({
